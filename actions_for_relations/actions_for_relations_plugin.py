...
class ActionsForRelationsPlugin(QObject):
    ...
    def __init__(self, iface: QgisInterface):
        ...
        self.translator.load(
            qgis_locale, "qgis-actions-for-relations", "_", locale_path
        )
        QCoreApplication.installTranslator(self.translator)

    def initGui(self):
        self.menu_action = QAction(
            self.tr("Set custom aggregate actions"), self.iface.mainWindow()
        )
    ...
    def load_relations(self):
        ...
        for layer in QgsProject.instance().mapLayers().values():
            ...
            menu_tree_main = QMenu(
                self.tr("Actions for relations"), self.iface.mainWindow()
            )

            menu_tree_show = QMenu(
                self.tr("Show referencing features for the selected features"),
                menu_tree_main,
            )
            menu_tree_add = QMenu(
                self.tr("Add referencing features for the selected features"),
                menu_tree_main,
            )
            ...
            for relation in relations:
                ...
                title = self.tr(
                    'Show features in referencing layer "{referencing}"'
                ).format(
                    referencing=relation.referencingLayer().name(),
                    referenced=relation.referencedLayer().name(),
                )
                self.create_menu_action(
                    menu_tree_show, title, relation, self.show_children
                )
                # batch insert
                ...
                self.create_menu_action(
                    menu_tree_add, title, relation, self.batch_insert
                )
        ...
        for relation in QgsProject.instance().relationManager().relations().values():
            ...
            self.add_map_layer_action(
                self.tr(
                    'Show referencing features in "{layer}" for relation "{rel}"'
            ...
            self.add_map_layer_action(
                self.tr(
                    'Add features in referencing layer "{layer}" for "relation "{rel}"'
                ).format(layer=relation.referencingLayer().name(), rel=relation.name()),
                relation,
                self.batch_insert,
            )
            ...
            for custom_aggregate in self.custom_aggregates:
                if custom_aggregate.relation_id == relation.id():
                    data = [custom_aggregate.aggregate, custom_aggregate.field]
                    self.add_map_layer_action(
                        custom_aggregate.title, relation, self.run_aggregate, data
                    )

    def add_map_layer_action(
        self, title: str, relation: QgsRelation, slot, data=None
    ) -> QgsMapLayerAction:
        ...
        if DEBUG:
            print(
                'Adding insert action for relation "{}" in layer "{}"'.format(
                    relation.name(), relation.referencedLayer().name()
                )
            )
    ...
    def create_menu_action(
        parent_menu: QMenu, title: str, relation: QgsRelation, slot, data=None
    ):
        # add legend context menu entry
        action = QAction(title, parent_menu)
        action.setData(relation.id())
        action.triggered.connect(
            lambda: slot(relation, relation.referencedLayer().selectedFeatures(), data)
        )
            ...
            parent_ids=", ".join(
                [
                    "{escape_referenced}{referenced}{escape_referenced}".format(
                        ...
                        escape_referenced="'"
                        if not relation.referencedLayer()
                        .fields()
                        .field(referenced)
                        .isNumeric()
                        else "",
    ...
    def batch_insert(self, relation: QgsRelation, features: [QgsFeature], data=None):
        ...
        for referenced_feature in features:
            ...
            for referencing_field, referenced_field in relation.fieldPairs().items():
                ...
                if not first_feature_created:
                    default_values[referencing_field_index] = referenced_feature[
                        referenced_field
                    ]
                    orignal_cfg[referencing_field_index] = layer.editorWidgetSetup(
                        referencing_field_index
                    )
                    layer.setEditorWidgetSetup(
                        referencing_field_index, QgsEditorWidgetSetup("Hidden", {})
                    )

                else:
                    # it has been created at previous iteration
                    referencing_feature[referencing_field_index] = referenced_feature[
                        referenced_field
                    ]

            if not first_feature_created:
                # show form for the feature with disabled widgets for the referencing fields
                ok, referencing_feature = self.iface.vectorLayerTools().addFeature(
                    layer, default_values, QgsGeometry()
                )
                if not ok:
                    break
        ...
        if ok:
            ...
            self.iface.messageBar().pushMessage(
                "Relation Batch Insert",
                self.tr(
                    "There was an error while inserting features, "
